...
def val(args, model, criterion, num_classes, csv_path, dataloader_val, epoch, loss_train_mean, iou_train_mean, writer):
    ...
    with torch.no_grad():             #禁止所有的需要梯度的张量进行梯度计算
        ...
        # p0, p1, p2, p3, p4, mpa = evaluator.mean_pixel_accuracy(hist)
        p0, p1, p2, mpa = evaluator.mean_pixel_accuracy(hist)
        # p0, p1, mpa = evaluator.mean_pixel_accuracy(hist)

        # r0, r1, r2, r3, r4, recall = evaluator.recall(hist)
        r0, r1, r2, recall = evaluator.recall(hist)
        # r0, r1, recall = evaluator.recall(hist)

        # precision = evaluator.precision(hist)

        # f0, f1, f2, f3, f4, F1 = evaluator.f1_score(hist)
        f0, f1, f2, F1 = evaluator.f1_score(hist)
        # f0, f1, F1 = evaluator.f1_score(hist)

        miou = evaluator.mean_intersection_over_union(hist)
        fwiou = evaluator.frequency_weighted_intersection_over_union(hist)

        # str_ = ("%15.5g;" * 25) % (epoch+1, loss_train_mean, loss_val_mean, pa, p0, p1, p2 ,p3, p4, f0, f1, f2, f3, f4, r0, r1, r2, r3, r4, mpa, recall, f1, iou_train_mean, miou, fwiou)
        str_ = ("%15.5g;" * 19) % (epoch+1, loss_train_mean, loss_val_mean, pa, p0, p1, p2, f0, f1, f2, r0, r1, r2, mpa, recall, F1, iou_train_mean, miou, fwiou)
        # str_ = ("%15.5g;" * 16) % (epoch+1, loss_train_mean, loss_val_mean, pa, p0, p1, f0, f1, r0, r1, mpa, recall, f1, iou_train_mean, miou, fwiou)
...
def train(args, model, csv_path, optimizer, criterion, dataloader_train, dataloader_val, exp_lr_scheduler):
    ...
    # 打印表头
    # s = ("%15s;" * 25) % ("epoch", "train_loss", "val_loss", "PA", "P0", "P1", "P2", "P3", "P4", "f0", "f1", "f2", "f3", "f4", "r0", "r1", "r2", "r3", "r4", "MPA", "Recall", "F1", "Train_Miou", "Val_Miou", "FWiou")
    s = ("%15s;" * 19) % ("epoch", "train_loss", "val_loss", "PA", "P0", "P1", "P2", "f0", "f1", "f2", "r0", "r1", "r2", "MPA", "Recall", "F1", "Train_Miou", "Val_Miou", "FWiou")
    # s = ("%15s;" * 16) % ("epoch", "train_loss", "val_loss", "PA", "P0", "P1", "f0", "f1", "r0", "r1", "MPA", "Recall", "F1", "Train_Miou", "Val_Miou", "FWiou")
    with open('result.txt', 'a') as f:
        f.write(s + '\n')
    with open(os.path.join(args.summary_path, args.dir_name, '') + args.dir_name + '_result.txt', 'a') as f:
        f.write(s + '\n')
